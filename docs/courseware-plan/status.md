...
- 课件源码库：当前 `src/` 下有 104 个课件目录，其中小学规划内按 ID 直接完成 84 个，初高中两轮规划内已上线 14 个。
...
| 第一轮物理 | 12 | 5 | 7 | `electric-mouse` 当前分类为 `science`，但纳入初中物理学习路径；`density-detective-lab`、`motion-tracker-lab`、`ohms-law-lab` 已完成 |
| 第一轮化学 | 7 | 2 | 5 | `mass-conservation-lab` 已完成；`ph-lab` 仍需从观察型 L1 升级到测量型 L3 |
| 第一轮生物/科学 | 3 | 2 | 1 | 暂沿用 `science` 分类；`microscope-cell-lab` 已完成 |
| 第一轮小计 | 26 | 11 | 15 | 存量存在不等于已通过新实验合同复核 |
