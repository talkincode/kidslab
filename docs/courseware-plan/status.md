...
| 科学 | 22 | 22 | 0 | `plant-xray`、`creature-workshop`、`eco-island`、`mini-pyramid` 与 `air-garage` 已按规划 ID 上线；`plant-lab` 仍是规划外养成实验 |
| 合计 | 84 | 84 | 0 | 另有 11 个规划外已上线课件 |
...
- [x] 空气车库 `air-garage`（`g3,g4`；杯子占位、气球天平和气球赛车证明空气占空间、有质量、压缩后能产生推力）
...
- [x] 有机分子工坊 `organic-builder-lab`（化学 · `g10,g12` · 第二轮 P0；现为单摆同款全屏观测壳。three.js 球棍拼装台按 VSEPR 布点，量角器读出 109.5°/120°，四瓶试剂填满 4×4 反应矩阵，L4 用连通性判定 C₂H₆O 与 C₄H₁₀ 的同分异构）
- [x] 圆周与卫星实验室 `circular-orbit-lab`（物理 · `g11` · 第二轮 P1；全屏 three.js 地球观测台 + 右侧可收拢面板，拧高度/水平速度看坠回、椭圆、圆轨、逃逸，读数入表并落在速度带上；同步轨道周期可现场读出，无预测题或通关锁）
- [x] DNA 复制实验室 `dna-replication-lab`（科学 · `g12` · 第二轮 P1；three.js 双螺旋沿复制叉解开，错配不成键，两条子代双链以颜色区分半保留，突变站追踪密码子→氨基酸，L4 找出同义突变）
- [x] 显微细胞实验室 `microscope-cell-lab`（科学 · `g7` · 第一轮 P0；three.js 复式显微镜，粗细准焦与物镜切换，视野直径随放大倍数反比缩小，洋葱表皮与口腔上皮对照构成 L3 闭环）
- [x] 抛体运动实验室 `projectile-motion-lab`（物理 · `g10` · 第一轮 P0；three.js 大炮靶场，平抛红球与同高自由落体黄球共用落地时钟，无阻力解析解对照数值轨迹，30°/8 m/s 穿过铁环构成 L4）
- [x] 抽样统计实验室 `statistics-sampling-lab`（数学 · `g10-g12` · 第一轮 P0；three.js 星河城四区，简单随机/分层/整群/便利抽样对照，便利样本稳定偏倚，普查线与 ±2 分钟分层命中构成 L3/L4）
...
| `plant-lab` 植物生长实验室 | science | primary/junior | 与已上线的 `plant-xray` 并存：前者是光照/水肥养成，后者是透视萌发、对照田和图鉴 |
...
- zh/en 双语、深浅主题、移动端触屏、桌面端布局都按 `AGENTS.md` 的质量规约检查过。
- 核心交互音效、成功/错误/通关反馈和静音控制已按 `AGENTS.md` 检查；纯观察型玩法没有错误状态时不强造失败音。
...
| 显微细胞实验室 | 高 | 已覆盖：桌面与触屏完成低倍对焦、记下 4×/10× 洋葱视野，再换口腔上皮完成植物/动物对照 | 已覆盖：40× 时粗准焦被拒绝且载物台位置不变；模糊视野不能写入记录 | 不适用：纯静态无角色 | 已覆盖：中途重载恢复记录；重新观察清空本课状态；错误操作后可继续记录 | `tests/e2e/microscope-cell-lab.spec.js`；`tests/unit/microscope-cell-lab.test.mjs` |
| 抛体运动实验室 | 高 | 已覆盖：桌面与触屏完成平抛/自由落体对照记录，再以 30°、8 m/s 穿过铁环完成闭环；中英与深浅主题切换无控制台错误 | 已覆盖：未发射与飞行中记录均被拒绝且笔记本不被写入 | 不适用：纯静态无角色 | 已覆盖：中途重载恢复记录；重新实验清空本课状态；错误操作后可继续发射并记录 | `tests/e2e/projectile-motion-lab.spec.js`；`tests/unit/projectile-motion-lab.test.mjs` |
| 圆周与卫星实验室 | 高 | 已覆盖：桌面与触屏完成圆轨发射与 92.4 min 读数、三种结局入表、速度带落点，以及一天周期圆轨观测；无预测题；中英与深浅主题切换无控制台错误 | 已覆盖：未发射与飞行中记录均被拒绝且笔记本不被写入 | 不适用：纯静态无角色 | 已覆盖：坠回后可收回再发射；三种结局入表后重载恢复；重新实验清空本课状态 | `tests/e2e/circular-orbit-lab.spec.js`；`tests/unit/circular-orbit.test.mjs`；`tests/unit/circular-orbit-lab.test.mjs` |
| 抽样统计实验室 | 高 | 已覆盖：桌面与触屏完成便利抽样与分层抽样记录，再经全市普查完成闭环；中英与深浅主题切换无控制台错误 | 已覆盖：未抽样记录与重复记录均被拒绝且笔记本不被写入 | 不适用：纯静态无角色 | 已覆盖：中途重载恢复记录；重新实验清空本课状态；错误操作后可继续抽样并记录 | `tests/e2e/statistics-sampling-lab.spec.js`；`tests/unit/statistics-sampling-lab.test.mjs` |
...
| 空气车库 | 中 | 已覆盖：桌面与触屏完成竖直扣杯纸巾不湿、扎破一只气球后天平倾斜，以及打满 6 下气的气球赛车通关 | 已覆盖：斜扣浸湿纸巾、未扎破/两只都扎破天平仍平、空车发车、气不足跑不完和打气过满爆胎后均可原地重试 | 不适用：纯静态无角色 | 已覆盖：错误后可继续；首关完成后的车间状态重载恢复；当前工位重来不丢已完成关卡 | `tests/e2e/air-garage.spec.js`；`tests/unit/air-garage.test.mjs` |
