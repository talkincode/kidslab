...
当前仓库尚无 E2E 测试框架，因此矩阵先如实记录缺口。缺口不得被解释为“已验证”；后续新增一级业务功能时，应优先补齐对应 E2E，无法补齐时必须在本矩阵和提交说明中明确保留缺口。
...
| 单个课件核心玩法 | 中 | 缺口 | 缺口 | 不适用：纯静态无角色 | 缺口：重开/撤销/错误操作需逐课件确认 | `AGENT.md` 提交前自检；需补每课件 E2E 或交互烟测 |
