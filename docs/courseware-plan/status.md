...
| 欧姆定律实验室 | 高 | 已覆盖：桌面与触屏自由调 U/R、记下两组以上 U-I 读数、图线斜率随电阻改变、中英/深浅主题与静音控件 | 已覆盖：电流表超量程拒绝记录并给出可修复文案；零电压仍可记录 | 不适用：纯静态无角色 | 已覆盖：改大量程后恢复读数；旧的错误接线存档迁移为正常接线且保留历史；清空记录不改当前旋钮 | `tests/e2e/ohms-law-lab.spec.js`；`tests/unit/ohms-law-lab.test.mjs` |
