...
本文沉淀"每天 23:00 自动开发新课件"这一循环的可复用机制，供外部触发器（cron / GitHub Actions）或人类以一句话触发（如「开发今天的课件」）发起每次周期。周期内的执行体（Agent / 人类）据此读取状态、自主选型、开发交付并回写进度。
...
Mira 无常驻定时器，因此循环必须由**外部触发信号**或**人类一句话触发**发起。

| 方式 | 触发信号 | 说明 |
| --- | --- | --- |
| A. GitHub Actions 定时 | cron（如 `.github/workflows/daily-dev.yml` 的 `schedule: cron: '0 15 * * *'`，UTC 15:00 = 东八区 23:00） | 推送 main 后自动构建发布，天然闭环 |
| B. 人类一句话 | 对 Agent 说「开发今天的课件」 | 最灵活的入口，适用于人工把关的周期 |
| C. 手动 | 直接运行本机制描述的步骤 | 便于调试与回放 |

> 当前仓库已有 `.github/workflows/deploy.yml`（push → build → E2E → 发布）。每日循环的第一步是读取状态、选型并提交 `src/`，提交本身即触发发布。若要完全无人值守，可在独立 workflow 中用 cron 在 23:00 拉起一个"开发并提交"的 job；**任何 cron 触发的自动提交都必须跑完 `npm run test:unit` 与 `npm run build`，不得仅推源码。**
