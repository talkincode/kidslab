...
- 打点按固定时间间隔 Δt = 0.10 s 采样，可以逐帧打点，也可以在播放时自动采样。这是教学动画，不是真实摄像机噪声。位置、速度和加速度都是模型值。
...
- [F6] 本课件水平轨道的速度恒为 0.40 m/s；斜面从静止释放，可选 10° / 20° / 30°。θ = 30° 时 a = 4.90 m/s²。这些是教学模型里的给定值，不是某次真实实验的测量结果。[S3] [S5]
