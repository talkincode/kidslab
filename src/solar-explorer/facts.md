...
- 天体显示半径使用平方根映射来保持大小排序但增强可见性；“压缩视图”对日心距离做平方根映射；“螺距缩放”只压缩太阳平移距离；“行星放大”只改变行星球体的显示半径。档案中的直径、距离、周期和速度数值不使用这些显示映射。
- 太阳绕太阳系质心的摆动按真实要素计算，但在画面上按太阳显示半径与真实半径之比同步放大，以保持“偏离几个太阳半径”这一比例关系可见；仪表盘给出的太阳半径倍数与公里数是未经放大的真实值。
- “星空视差”使用程序化近、中、远星层，并将层间深度与位移显著增强，让太阳系前进方向在有限时间和画幅内可见；它是运动方向提示，不代表真实恒星距离或可见视差角。视差强度独立于“螺距缩放”，远景星空仍作为近似无限远的惯性天球。
...
- [F11] 把同一个质心平移向量同时加到太阳和全部行星，不改变任一行星相对太阳的位置；质心、邻近恒星和银河三个参考系展示的是同一组轨道在不同整体运动下的轨迹。[S10]
- [F12] 太阳系质心由太阳与各行星的质量矩共同决定，太阳绕该点运动而非固定在原点；课件用 JPL 的太阳/行星系质量比求 B=Σmᵢrᵢ/Σmᵢ，取太阳相对质心的位移。[S12]
- [F13] 仅考虑木星时，日木质心位于太阳中心约 1.07 个太阳半径处，即落在太阳表面之外；计入全部行星、且巨行星大致同向时，太阳系质心与太阳中心的距离可接近 2 个太阳半径。[S13] [S14]
...
- [S12] NASA/JPL Solar System Dynamics：《Astrodynamic Parameters》（DE440 的太阳与行星系 GM；课件以 GM☉/GM行星系换算质量比）；https://ssd.jpl.nasa.gov/astro_par.html
- [S13] NASA Space Place：《What Is a Barycenter?》；https://spaceplace.nasa.gov/barycenter/en/
- [S14] Meeus, J. (1997)：《Mathematical Astronomy Morsels》第 16 章 “The Sun's Motion Around the Barycenter”
