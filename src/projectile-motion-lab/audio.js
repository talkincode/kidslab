...
export function createAudio(storageKey = 'kidslab.projectile-motion-lab') {
  const store = {
    get(key, fallback) {
      try {
  ...
  function launch() {
    if (!readyForSfx()) return;
    const now = ctx.currentTime;
    tone({ type: 'triangle', from: 240, to: 90, at: now, duration: 0.12, level: 0.07 });
    tone({ type: 'sine', from: 420, to: 180, at: now + 0.02, duration: 0.1, level: 0.04 });
  }

  function land() {
    if (!readyForSfx()) return;
    const now = ctx.currentTime;
    tone({ type: 'triangle', from: 160, to: 70, at: now, duration: 0.14, level: 0.06 });
  }
