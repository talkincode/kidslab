...
function tone(){if(muted)return;try{const ctx=new (window.AudioContext||window.webkitAudioContext)();const oscillator=ctx.createOscillator(),gain=ctx.createGain();oscillator.frequency.value=600;gain.gain.value=.04;oscillator.connect(gain).connect(ctx.destination);oscillator.start();oscillator.stop(ctx.currentTime+.08);oscillator.onended=()=>ctx.close();}catch{}}
...
$('soundBtn').onclick=()=>{muted=!muted;localStorage.setItem(`${SAVE_KEY}.sound`,muted);render();};
