...
2. **文案说人话，而且要克制**
   ...
   - 好课件的第一印象是想伸手玩,不是先看说明书;文字密度过大就是失败
...
5. **布局合理,少滚动,舞台优先**
   ...
   - 首屏必须看见能玩的东西;说明文字默认收进轻提示或交互后揭示,别把玩法区挤成说明书附录
...
7. **先玩后说,提示克制**
   - 同时可见的说明最多 1 句动作邀请(十几个字就够),禁止把「任务说明 + 状态复述 + 规则卡 + 知识点 + 公式」叠在首屏
   - 规则、原理、公式、图例默认收起;靠第一次操作、第一次对/错、轻提示按钮、或卡住约 30 秒再出现
   - 知识点是玩出来的奖励,不是进门考卷;禁止在开局文案里写出答案或完整公式
...
8. **交互音效有语义,默认可控**
   ...
   - 不得在首次用户手势前自动播放。`AudioContext` 创建或 `resume()` 失败、浏览器不支持音频时要静默降级,不能影响玩法或产生控制台错误;音频资源必须本地存放,不得依赖 CDN
   - 拖拽只在拾起、进入有效落点、吸附成功、错误回弹等状态边界发声,禁止每个 `pointermove` 创建音源;碰撞和连续仿真必须做进入边界判定或节流/冷却,禁止在 `requestAnimationFrame` 或物理帧里持续堆叠音源
   - 用 `GainNode` 和短包络控制响度、消除爆音,避免多个音效叠加削波;循环或环境音在静音、页面隐藏、离开课件时应停止或将增益降为零
