...
```
index.html          # 主站入口
manifest.webmanifest # PWA manifest(fullscreen 壳 + 图标)
sw.js               # ⛔ Service Worker 构建产物 —— 由 build 从 scripts/sw.js 生成,不要手改
assets/             # 主站 css/js/字体/logo(手写,非构建产物;icons/ 由 npm run icons 生成)
src/<id>/           # ✏️ 课件源码 —— 在这里开发
...
scripts/build.mjs   # 构建/校验逻辑(分类、学段白名单在此;末尾生成 index.html 指纹与 sw.js)
scripts/sw.js       # Service Worker 源码模板(缓存策略在此)
scripts/icons.mjs   # PWA 图标生成:logo.svg 变更后运行 npm run icons 并提交 assets/icons/
...
tests/e2e/          # Playwright smoke:主站筛选与全部清单课件 + PWA/离线验收
playwright.config.js # 桌面 1280×800 + 手机 375×667 两档配置
cloudflare/analytics/  # 埋点接收 Worker(push 时经 deploy-analytics.yml 自动部署)
.github/workflows/deploy.yml  # push main → 自动构建发布 GitHub Pages + Cloudflare Pages
.github/workflows/deploy-analytics.yml  # cloudflare/analytics 变更 → 自动部署 Worker
```

**关键规则:改课件只改 `src/`,然后 `npm run build` 重新生成 `courseware/`,两者一起提交。改主站 assets 或 scripts/sw.js 后同样要重新 build(index.html 指纹与 sw.js 会更新),产物一起提交。**

## PWA 与离线缓存

主站是可安装 PWA(`manifest.webmanifest`,`display: fullscreen`)。缓存分层见 `scripts/sw.js` 模板注释:

- **主站壳**(index.html + assets + 课件清单)按预缓存内容整体 hash 版本化预缓存,发版自动换代清理
- **课件** network-first + cache-on-visit(`kidslab-courseware` 运行时缓存):在线永远最新(发版不"粘死"),玩过的课件断网可重玩;主站卡片据 Cache API 显示「可离线」角标
- **更新保守**:不 `skipWaiting`,新 SW 等所有页面关闭后接管,避免孩子玩到一半被刷新
- SW 只拦截同源 GET;analytics 上报离线时自然丢弃(尽力而为语义,无离线队列)
- 触控:主站 CSS 与注入式 SDK 均设 `touch-action: manipulation`,消除双击缩放手势仲裁延迟;课件内 `touch-action: none` 的拖拽画布不受影响
- 本地离线验证:`npm run preview` 访问主站与某课件后停掉服务器,重开页面应能从缓存加载;e2e 由 `tests/e2e/pwa.spec.js` 覆盖
...
每个新课件必须有 `src/<id>/facts.md`。它是审核知识正确性的靶子，不会复制到运行时 `courseware/` 产物。格式固定为:
...
## 知识断言
- [F1] 一条可独立核对、且课件实际教授或依赖的事实/公式/规则。[S1]

## 来源
- [S1] 权威来源名称、章节或页码；官方链接（如有）
```

- 每条断言必须引用至少一个已定义的 `[S编号]`;有争议的表述要给精确章节或页码
- 公式要写单位、变量范围和适用条件;不要把近似值写成精确相等
- 不列课件没有教授或依赖的背景知识,不以 AI 生成内容代替人工核对来源
- `scripts/facts-legacy.json` 是存量债务清单:修改存量课件的知识内容时,必须补 `facts.md` 并移出该清单
- 构建会拒绝既无 `facts.md`、也未登记为存量债务的课件;已有断言的课件不能继续留在债务清单
...
push 到 `main` 自动触发 workflow:`npm ci` → 无 analytics 构建 → 安装 Chromium 并运行 `npm run test:e2e` → 按仓库变量重新生成发布产物 → 组装同一份 `_site`(index.html + manifest.webmanifest + sw.js + assets + courseware)→ 发布 GitHub Pages 与 Cloudflare Pages。Cloudflare Pages 项目名固定为 `kidslab`，自定义入口为 `https://kidslab.talkincode.net/`；首次启用需在 Cloudflare 创建 Pages 项目，Production branch 设为 `main`，配置 `CLOUDFLARE_API_TOKEN`（需 Account → Cloudflare Pages → Edit；多账号时同时配置 `CLOUDFLARE_ACCOUNT_ID`），在 Pages 项目 Custom domains 绑定 `kidslab.talkincode.net`，并确认 DNS 记录 `kidslab CNAME kidslab.pages.dev` 已生效。国内可达性必须依据多地区/多运营商实测记录更新 README，实测通过前不得承诺“国内稳定访问”。无需手动操作。
