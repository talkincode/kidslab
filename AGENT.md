...
push 到 `main` 自动触发 workflow:`npm ci` → 无 analytics 构建 → 安装 Chromium 并运行 `npm run test:e2e` → 按仓库变量重新生成发布产物 → 组装 `_site/`(index.html + manifest.webmanifest + sw.js + assets + courseware)→ 发布 GitHub Pages。无需手动操作。
