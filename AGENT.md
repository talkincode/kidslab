...
没有测试和 lint;`npm run build` 即校验,改动后必须运行并确保通过(退出码 0)。课件改动提交前,还必须逐项过一遍下方「提交前自检清单」。
...
1. 从模板起步:复制 `docs/courseware-template/` 四个文件到 `src/<courseware-id>/`(目录名即 `id`),模板已内置双语(`kidslab.lang`)与深浅主题(`kidslab.theme`)机制,用法见 `docs/courseware-template/README.md`
...
当前仓库尚无 E2E 测试框架;因此未覆盖项必须在验收矩阵中保留为"缺口",不得写成已验证。若本轮无法补齐 E2E,提交说明要明确该验证缺口仍存在。
...
- **中英双语**:界面文案提供 zh/en,读写 `localStorage: kidslab.lang` 与主站同步语言偏好
- **主题适配**:遵循深浅主题;主站配色变量见 `assets/css/app.css`
...
push 到 `main` 自动触发 workflow:`npm ci` → `npm run build` → 组装 `_site/`(index.html + assets + courseware)→ 发布 GitHub Pages。无需手动操作。
